...
{
    public class HaiKangService_Linux : IHaiKangService
    {
        ...
        private CHCNetSDK_Linux.REALDATACALLBACK RealData = null;
        public CHCNetSDK_Linux.NET_DVR_DEVICEINFO_V30 DeviceInfo;
        public CHCNetSDK_Linux.NET_DVR_IPPARACFG_V40 m_struIpParaCfgV40;
        public CHCNetSDK_Linux.NET_DVR_STREAM_MODE m_struStreamMode;
        public CHCNetSDK_Linux.NET_DVR_IPCHANINFO m_struChanInfo;
        public CHCNetSDK_Linux.NET_DVR_PU_STREAM_URL m_struStreamURL;
        public CHCNetSDK_Linux.NET_DVR_IPCHANINFO_V40 m_struChanInfoV40;
        private PlayCtrl_Linux.DECCBFUN m_fDisplayFun = null;
        public delegate void MyDebugInfo(string str);

        //MonitorService monitorService = new MonitorService();
        public HaiKangService_Linux()
        {
            m_bInitSDK = CHCNetSDK_Linux.NET_DVR_Init();
            if (m_bInitSDK == false)
            {
            ...
            else
            {
                ...
                CHCNetSDK_Linux.NET_DVR_SetLogToFile(1, "/SdkLog1/", true);
        ...
        public override void Logout()
        {
            if (m_lUserID >= 0)
            {
                ...
                if (!CHCNetSDK_Linux.NET_DVR_Logout(m_lUserID))
                {
                    iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
        ...
        public override void Login(string DVRIPAddress, string DVRPort, string DVRUserName, string DVRPassword,int ChannelNum)
        {
            if (m_lUserID < 0)
            {
                //string DVRIPAddress = "10.0.102.220"; //设备IP地址或者域名 Device IP
                ...
                //string DVRPassword = "Zrh#2020";//设备登录密码 Password to login



                //登录设备 Login the device
                m_lUserID = CHCNetSDK_Linux.NET_DVR_Login_V30(DVRIPAddress, DVRPortNumber, DVRUserName, DVRPassword, ref DeviceInfo);
                if (m_lUserID < 0)
                {
                    iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
                ...
                else
                {
                    ...
                    //登录成功
                    DebugInfo("Linux NET_DVR_Login_V30 succ!");
        ...
        {
            #region MyRegion
            //Console.WriteLine($"RealDataCallBack回调回调函数：lRealHandle={lRealHandle},dwDataType={dwDataType},pBuffer={pBuffer},dwBufSize={dwBufSize},dwUser={pUser}");
            //下面数据处理建议使用委托的方式
            //MyDebugInfo AlarmInfo = new MyDebugInfo(DebugInfo);
            //switch (dwDataType)
            //{
            //    case CHCNetSDK_Linux.NET_DVR_SYSHEAD:     // sys head
            //        if (dwBufSize > 0)
            //        {
            //            if (m_lPort >= 0)
            //            {
            ...
            //            if (!PlayCtrl_Linux.PlayM4_GetPort(ref m_lPort))
            //            {
            //                iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
            ...
            //            if (!PlayCtrl_Linux.PlayM4_SetStreamOpenMode(m_lPort, PlayCtrl_Linux.STREAME_REALTIME))
            //            {
            //                iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
            ...
            //            if (!PlayCtrl_Linux.PlayM4_OpenStream(m_lPort, pBuffer, dwBufSize, 2 * 1024 * 1024))
            //            {
            //                iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
            ...
            //            if (!PlayCtrl_Linux.PlayM4_SetDisplayBuf(m_lPort, 15))
            //            {
            //                iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
            ...
            //            if (!PlayCtrl_Linux.PlayM4_SetOverlayMode(m_lPort, 0, 0/* COLORREF(0)*/)) //play off screen 
            //            {
            //                iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
            ...
            //            m_fDisplayFun = new PlayCtrl_Linux.DECCBFUN(DecCallbackFUN);
            //            if (!PlayCtrl_Linux.PlayM4_SetDecCallBackEx(m_lPort, m_fDisplayFun, IntPtr.Zero, 0))
            //            {
            //                //this.BeginInvoke(AlarmInfo, "PlayM4_SetDisplayCallBack fail");
            //            }

            //            //开始解码 Start to play                       
            //            if (!PlayCtrl_Linux.PlayM4_Play(m_lPort, m_ptrRealHandle))
            //            {
            //                iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
            ...
            //        }
            //        break;
            //    case CHCNetSDK_Linux.NET_DVR_STREAMDATA:     // video stream data
            //        if (dwBufSize > 0 && m_lPort != -1)
            //        {
            //            for (int i = 0; i < 999; i++)
            //            {
            //                //送入码流数据进行解码 Input the stream data to decode
            //                if (!PlayCtrl_Linux.PlayM4_InputData(m_lPort, pBuffer, dwBufSize))
            //                {
            //                    iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
        ...
        {
            CHCNetSDK_Linux.NET_DVR_JPEGPARA lpJpegPara = new CHCNetSDK_Linux.NET_DVR_JPEGPARA();
            ...
            if (!CHCNetSDK_Linux.NET_DVR_CaptureJPEGPicture_NEW(m_lUserID, iChannelNum[(int)iSelIndex], ref lpJpegPara, byJpegPicBuffer, iBuffSize, ref dwSizeReturned))
            {
                iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
        ...
        //解码回调函数
        private void DecCallbackFUN(int nPort, IntPtr pBuf, int nSize, ref PlayCtrl_Linux.FRAME_INFO pFrameInfo, int nReserved1, int nReserved2)
        {
        ...
        public override void StopShow()
        {
            ...
            {
                //停止预览 Stop live view 
                if (!CHCNetSDK_Linux.NET_DVR_StopRealPlay(m_lRealHandle))
                {
                    iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
                ...
                if (m_lPort >= 0)
                {
                    if (!PlayCtrl_Linux.PlayM4_Stop(m_lPort))
                    {
                        iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
                        str = "PlayM4_Stop failed, error code= " + iLastErr;
                        DebugInfo(str);
                    }
                    if (!PlayCtrl_Linux.PlayM4_CloseStream(m_lPort))
                    {
                        iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
                        str = "PlayM4_CloseStream failed, error code= " + iLastErr;
                        DebugInfo(str);
                    }
                    if (!PlayCtrl_Linux.PlayM4_FreePort(m_lPort))
                    {
                        iLastErr = PlayCtrl_Linux.PlayM4_GetLastError(m_lPort);
        ...
        public override void Show()
        {
            ...
            if (m_lRealHandle < 0)
            {
                CHCNetSDK_Linux.NET_DVR_PREVIEWINFO lpPreviewInfo = new CHCNetSDK_Linux.NET_DVR_PREVIEWINFO();
                ...
                //    m_lRealHandle = CHCNetSDK_Linux.NET_DVR_RealPlay_V40(m_lUserID, ref lpPreviewInfo, null/*RealData*/, pUser);
                //}
                //else
                {
                    //lpPreviewInfo.hPlayWnd = IntPtr.Zero;//预览窗口 live view window
                    //m_ptrRealHandle = RealPlayWnd.Handle;
                    RealData = new CHCNetSDK_Linux.REALDATACALLBACK(RealDataCallBack);//预览实时流回调函数 real-time stream callback function 
                    m_lRealHandle = CHCNetSDK_Linux.NET_DVR_RealPlay_V40(m_lUserID, ref lpPreviewInfo, RealData, pUser);
                }
                ...
                {
                    //预览成功
                    DebugInfo("Linux NET_DVR_RealPlay_V40 succ!");
        ...
        public void InfoIPChannel()
        {
            ...
            if (!CHCNetSDK_Linux.NET_DVR_GetDVRConfig(m_lUserID, CHCNetSDK_Linux.NET_DVR_GET_IPPARACFG_V40, iGroupNo, ptrIpParaCfgV40, dwSize, ref dwReturn))
            {
                iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
            ...
            else
            {
                ...
                m_struIpParaCfgV40 = (CHCNetSDK_Linux.NET_DVR_IPPARACFG_V40)Marshal.PtrToStructure(ptrIpParaCfgV40, typeof(CHCNetSDK_Linux.NET_DVR_IPPARACFG_V40));
                ...
                for (i = 0; i < iDChanNum; i++)
                {
                    ...
                    switch (byStreamType)
                    {
                        ...
                        case 0:
                            ...
                            m_struChanInfo = (CHCNetSDK_Linux.NET_DVR_IPCHANINFO)Marshal.PtrToStructure(ptrChanInfo, typeof(CHCNetSDK_Linux.NET_DVR_IPCHANINFO));
                        ...
                        case 4:
                            ...
                            m_struStreamURL = (CHCNetSDK_Linux.NET_DVR_PU_STREAM_URL)Marshal.PtrToStructure(ptrStreamURL, typeof(CHCNetSDK_Linux.NET_DVR_PU_STREAM_URL));
                        ...
                        case 6:
                            ...
                            m_struChanInfoV40 = (CHCNetSDK_Linux.NET_DVR_IPCHANINFO_V40)Marshal.PtrToStructure(ptrChanInfoV40, typeof(CHCNetSDK_Linux.NET_DVR_IPCHANINFO_V40));
        ...
        {
            CHCNetSDK_Linux.NET_DVR_Cleanup();
        }
        ...
        public override void TurnRoundDown(int speed, int direction)
        {
            ...
            {
                if (direction == 0)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.TILT_UP, 0, (uint)speed);
                else if (direction == 1)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.TILT_DOWN, 0, (uint)speed);
                else if (direction == 2)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.PAN_LEFT, 0, (uint)speed);
                else if (direction == 3)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.PAN_RIGHT, 0, (uint)speed);
                else if (direction == 4)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.UP_LEFT, 0, (uint)speed);
                else if (direction == 5)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.UP_RIGHT, 0, (uint)speed);
                else if (direction == 6)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.DOWN_LEFT, 0, (uint)speed);
                else if (direction == 7)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.DOWN_RIGHT, 0, (uint)speed);
                else if (direction == 8)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.PAN_AUTO, 0, (uint)speed);
            }
            else
            {
                if (direction == 0)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.TILT_UP, 0, (uint)speed);
                else if (direction == 1)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.TILT_DOWN, 0, (uint)speed);
                else if (direction == 2)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.PAN_LEFT, 0, (uint)speed);
                else if (direction == 3)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.PAN_RIGHT, 0, (uint)speed);
                else if (direction == 4)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.UP_LEFT, 0, (uint)speed);
                else if (direction == 5)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.UP_RIGHT, 0, (uint)speed);
                else if (direction == 6)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.DOWN_LEFT, 0, (uint)speed);
                else if (direction == 7)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.DOWN_RIGHT, 0, (uint)speed);
                else if (direction == 8)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.PAN_AUTO, 0, (uint)speed);
            }
        }
        public override void TurnRoundUp(int speed, int direction)
        {
            ...
            {
                if (direction == 0)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.TILT_UP, 1, (uint)speed);

                else if (direction == 1)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.TILT_DOWN, 1, (uint)speed);
                else if (direction == 2)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.PAN_LEFT, 1, (uint)speed);
                else if (direction == 3)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.PAN_RIGHT, 1, (uint)speed);
                else if (direction == 4)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.UP_LEFT, 1, (uint)speed);
                else if (direction == 5)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.UP_RIGHT, 1, (uint)speed);
                else if (direction == 6)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.DOWN_LEFT, 1, (uint)speed);
                else if (direction == 7)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.DOWN_RIGHT, 1, (uint)speed);
                else if (direction == 8)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed(m_lRealHandle, CHCNetSDK_Linux.PAN_AUTO, 1, (uint)speed);

            }
            else
            {
                if (direction == 0)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.TILT_UP, 1, (uint)speed);
                else if (direction == 1)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.TILT_DOWN, 1, (uint)speed);
                else if (direction == 2)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.PAN_LEFT, 1, (uint)speed);
                else if (direction == 3)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.PAN_RIGHT, 1, (uint)speed);
                else if (direction == 4)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.UP_LEFT, 1, (uint)speed);
                else if (direction == 5)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.UP_RIGHT, 1, (uint)speed);
                else if (direction == 6)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.DOWN_LEFT, 1, (uint)speed);
                else if (direction == 7)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.DOWN_RIGHT, 1, (uint)speed);
                else if (direction == 8)
                    CHCNetSDK_Linux.NET_DVR_PTZControlWithSpeed_Other(m_lUserID, iChannelNum[(int)iSelIndex], CHCNetSDK_Linux.PAN_AUTO, 1, (uint)speed);
            }
        }
        ...
        private CHCNetSDK_Linux.EXCEPYIONCALLBACK m_fExceptionCB = null;
        ...
        private CHCNetSDK_Linux.MSGCallBack_V31 m_falarmData_V31 = null;
        public bool MsgCallback_V31(int lCommand, ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
        ...
        {
            //设置透传报警信息类型
            CHCNetSDK_Linux.NET_DVR_LOCAL_GENERAL_CFG struLocalCfg = new CHCNetSDK_Linux.NET_DVR_LOCAL_GENERAL_CFG();
            ...
            if (!CHCNetSDK_Linux.NET_DVR_SetSDKLocalCfg(17, ptrLocalCfg))  //NET_DVR_LOCAL_CFG_TYPE_GENERAL
            {
                iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
            ...
            {
                m_fExceptionCB = new CHCNetSDK_Linux.EXCEPYIONCALLBACK(cbExceptionCB);
            }
            CHCNetSDK_Linux.NET_DVR_SetExceptionCallBack_V30(0, IntPtr.Zero, m_fExceptionCB, IntPtr.Zero);


            //设置报警回调函数
            if (m_falarmData_V31 == null)
            {
                m_falarmData_V31 = new CHCNetSDK_Linux.MSGCallBack_V31(MsgCallback_V31);
            }
            CHCNetSDK_Linux.NET_DVR_SetDVRMessageCallBack_V31(m_falarmData_V31, IntPtr.Zero);
        }
        public override ReturnVideo SetAlarm()
        {
            InitAlarm();
            CHCNetSDK_Linux.NET_DVR_SETUPALARM_PARAM struAlarmParam = new CHCNetSDK_Linux.NET_DVR_SETUPALARM_PARAM();
            struAlarmParam.dwSize = (uint)Marshal.SizeOf(struAlarmParam);
            struAlarmParam.byLevel = 1; //0- 一级布防,1- 二级布防
            struAlarmParam.byAlarmInfoType = 1;//智能交通设备有效，新报警信息类型
            struAlarmParam.byFaceAlarmDetection = 1;//1-人脸侦测

            var m_lAlarmHandle = CHCNetSDK_Linux.NET_DVR_SetupAlarmChan_V41(m_lUserID, ref struAlarmParam);
            if (m_lAlarmHandle < 0)
            {
                iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
        ...
        public override ReturnVideo CloseAlarm()
        {
            if (m_lAlarmHandle > 0)
            {
                if (!CHCNetSDK_Linux.NET_DVR_CloseAlarmChan_V30(m_lUserID))
                {
                    iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
        ...
        public override ReturnVideo AlarmStartListen()
        {
            try
            {
                ...
                {
                   m_falarmData = new CHCNetSDK_Linux.MSGCallBack(MsgCallback);
                }

                iListenHandle = CHCNetSDK_Linux.NET_DVR_StartListen_V30(sLocalIP, wLocalPort, m_falarmData, IntPtr.Zero);
                if (iListenHandle < 0)
                {
                    iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
            ...
            {
                Console.WriteLine("AlarmStartListen:" + ex.Message);
            }
            return null;
        }
        ...
        {
            if (!CHCNetSDK_Linux.NET_DVR_StopListen_V30(iListenHandle))
            {
            ...
            {
                return new ReturnVideo { Success = false, Msg = "停止监听！" };
            }
        }

        public override void Zoom(int isStart, int isIn)
        {
            if (!CHCNetSDK_Linux.NET_DVR_PTZControl(m_lRealHandle, isIn == 1 ? (uint)CHCNetSDK_Linux.ZOOM_IN : (uint)CHCNetSDK_Linux.ZOOM_OUT, isStart == 1 ? (uint)0 : (uint)1))
            {
                iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
                var strErr = "停止监听失败，错误号：" + iLastErr; //撤防失败，输出错误号
                Console.WriteLine(strErr);
        ...
        {
            if (!CHCNetSDK_Linux.NET_DVR_PTZCruise(m_lRealHandle, isStart == 1 ? (uint)CHCNetSDK_Linux.RUN_SEQ : (uint)CHCNetSDK_Linux.STOP_SEQ, (byte)byCruiseRoute, 0, 0))
            {
                iLastErr = CHCNetSDK_Linux.NET_DVR_GetLastError();
        ...
        public CHCNetSDK_Linux.MSGCallBack m_falarmData = null;
        /*********************************************************
        ...
        Return:	
        **********************************************************/
        public void MsgCallback(int lCommand, ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            try
            {
                //通过lCommand来判断接收到的报警信息类型，不同的lCommand对应不同的pAlarmInfo内容
        ...
        public void AlarmMessageHandle(int lCommand, ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            //通过lCommand来判断接收到的报警信息类型，不同的lCommand对应不同的pAlarmInfo内容
            switch (lCommand)
            {
                case CHCNetSDK_Linux.COMM_ALARM: //(DS-8000老设备)移动侦测、视频丢失、遮挡、IO信号量等报警信息
                    ProcessCommAlarm(ref pAlarmer, pAlarmInfo, dwBufLen, pUser);
                    break;
                case CHCNetSDK_Linux.COMM_ALARM_V30://移动侦测、视频丢失、遮挡、IO信号量等报警信息
                    ProcessCommAlarm_V30(ref pAlarmer, pAlarmInfo, dwBufLen, pUser);
                    break;
                case CHCNetSDK_Linux.COMM_ALARM_RULE://进出区域、入侵、徘徊、人员聚集等行为分析报警信息
                    ProcessCommAlarm_RULE(ref pAlarmer, pAlarmInfo, dwBufLen, pUser);
                    break;
                case CHCNetSDK_Linux.COMM_UPLOAD_PLATE_RESULT://交通抓拍结果上传(老报警信息类型)
                    ProcessCommAlarm_Plate(ref pAlarmer, pAlarmInfo, dwBufLen, pUser);
                    break;
                case CHCNetSDK_Linux.COMM_ITS_PLATE_RESULT://交通抓拍结果上传(新报警信息类型)
                ...
                case CHCNetSDK_Win64.COMM_UPLOAD_VIDEO_INTERCOM_EVENT://可视对讲事件记录信息
                    ...
                    break;
                case CHCNetSDK_Linux.COMM_ALARM_ACS://门禁主机报警上传
                    ProcessCommAlarm_AcsAlarm(ref pAlarmer, pAlarmInfo, dwBufLen, pUser);
                    break;
                case CHCNetSDK_Linux.COMM_ID_INFO_ALARM://身份证刷卡信息上传
                ...
                case CHCNetSDK_Win64.COMM_UPLOAD_AIOP_PICTURE://设备支持AI开放平台接入，上传图片检测数据
                    ...
                    break;
                case CHCNetSDK_Linux.COMM_ISAPI_ALARM://ISAPI报警信息上传
        ...
        public void ProcessCommAlarm(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_ALARMINFO struAlarmInfo = new CHCNetSDK_Linux.NET_DVR_ALARMINFO();

            struAlarmInfo = (CHCNetSDK_Linux.NET_DVR_ALARMINFO)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_ALARMINFO));
        ...
        private void ProcessCommAlarm_V30(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_ALARMINFO_V30 struAlarmInfoV30 = new CHCNetSDK_Linux.NET_DVR_ALARMINFO_V30();

            struAlarmInfoV30 = (CHCNetSDK_Linux.NET_DVR_ALARMINFO_V30)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_ALARMINFO_V30));
        ...
        CHCNetSDK_Linux.NET_VCA_TRAVERSE_PLANE m_struTraversePlane = new CHCNetSDK_Linux.NET_VCA_TRAVERSE_PLANE();
        CHCNetSDK_Linux.NET_VCA_AREA m_struVcaArea = new CHCNetSDK_Linux.NET_VCA_AREA();
        CHCNetSDK_Linux.NET_VCA_INTRUSION m_struIntrusion = new CHCNetSDK_Linux.NET_VCA_INTRUSION();

        private int iFileNumber = 0; //保存的文件个数


        private void ProcessCommAlarm_RULE(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_VCA_RULE_ALARM struRuleAlarmInfo = new CHCNetSDK_Linux.NET_VCA_RULE_ALARM();
            struRuleAlarmInfo = (CHCNetSDK_Linux.NET_VCA_RULE_ALARM)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_VCA_RULE_ALARM));
            ...
            switch (struRuleAlarmInfo.struRuleInfo.wEventTypeEx)
            {
                case (ushort)CHCNetSDK_Win64.VCA_RULE_EVENT_TYPE_EX.ENUM_VCA_EVENT_TRAVERSE_PLANE:
                    ...
                    m_struTraversePlane = (CHCNetSDK_Linux.NET_VCA_TRAVERSE_PLANE)Marshal.PtrToStructure(ptrTraverseInfo, typeof(CHCNetSDK_Linux.NET_VCA_TRAVERSE_PLANE));
                ...
                case (ushort)CHCNetSDK_Win64.VCA_RULE_EVENT_TYPE_EX.ENUM_VCA_EVENT_ENTER_AREA:
                    ...
                    m_struVcaArea = (CHCNetSDK_Linux.NET_VCA_AREA)Marshal.PtrToStructure(ptrEnterInfo, typeof(CHCNetSDK_Linux.NET_VCA_AREA));
                ...
                case (ushort)CHCNetSDK_Win64.VCA_RULE_EVENT_TYPE_EX.ENUM_VCA_EVENT_EXIT_AREA:
                    ...
                    m_struVcaArea = (CHCNetSDK_Linux.NET_VCA_AREA)Marshal.PtrToStructure(ptrExitInfo, typeof(CHCNetSDK_Linux.NET_VCA_AREA));
                ...
                case (ushort)CHCNetSDK_Win64.VCA_RULE_EVENT_TYPE_EX.ENUM_VCA_EVENT_INTRUSION:
                    ...
                    m_struIntrusion = (CHCNetSDK_Linux.NET_VCA_INTRUSION)Marshal.PtrToStructure(ptrIntrusionInfo, typeof(CHCNetSDK_Linux.NET_VCA_INTRUSION));
        ...
        private void ProcessCommAlarm_Plate(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_PLATE_RESULT struPlateResultInfo = new CHCNetSDK_Linux.NET_DVR_PLATE_RESULT();
            uint dwSize = (uint)Marshal.SizeOf(struPlateResultInfo);

            struPlateResultInfo = (CHCNetSDK_Linux.NET_DVR_PLATE_RESULT)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_PLATE_RESULT));
        ...
        private void ProcessCommAlarm_ITSPlate(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_ITS_PLATE_RESULT struITSPlateResult = new CHCNetSDK_Linux.NET_ITS_PLATE_RESULT();
            uint dwSize = (uint)Marshal.SizeOf(struITSPlateResult);

            struITSPlateResult = (CHCNetSDK_Linux.NET_ITS_PLATE_RESULT)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_ITS_PLATE_RESULT));
        ...
        private void ProcessCommAlarm_TPSStatInfo(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_TPS_STATISTICS_INFO struTPSStatInfo = new CHCNetSDK_Linux.NET_DVR_TPS_STATISTICS_INFO();
            uint dwSize = (uint)Marshal.SizeOf(struTPSStatInfo);

            struTPSStatInfo = (CHCNetSDK_Linux.NET_DVR_TPS_STATISTICS_INFO)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_TPS_STATISTICS_INFO));
        ...
        CHCNetSDK_Linux.UNION_STATFRAME m_struStatFrame = new CHCNetSDK_Linux.UNION_STATFRAME();
        CHCNetSDK_Linux.UNION_STATTIME m_struStatTime = new CHCNetSDK_Linux.UNION_STATTIME();
        private void ProcessCommAlarm_PDC(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_PDC_ALRAM_INFO struPDCInfo = new CHCNetSDK_Linux.NET_DVR_PDC_ALRAM_INFO();
            uint dwSize = (uint)Marshal.SizeOf(struPDCInfo);
            struPDCInfo = (CHCNetSDK_Linux.NET_DVR_PDC_ALRAM_INFO)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_PDC_ALRAM_INFO));
            ...
            {
                m_struStatFrame = (CHCNetSDK_Linux.UNION_STATFRAME)Marshal.PtrToStructure(ptrPDCUnion, typeof(CHCNetSDK_Linux.UNION_STATFRAME));
            ...
            {
                m_struStatTime = (CHCNetSDK_Linux.UNION_STATTIME)Marshal.PtrToStructure(ptrPDCUnion, typeof(CHCNetSDK_Linux.UNION_STATTIME));
        ...
        private void ProcessCommAlarm_TPSRealInfo(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_TPS_REAL_TIME_INFO struTPSInfo = new CHCNetSDK_Linux.NET_DVR_TPS_REAL_TIME_INFO();
            uint dwSize = (uint)Marshal.SizeOf(struTPSInfo);

            struTPSInfo = (CHCNetSDK_Linux.NET_DVR_TPS_REAL_TIME_INFO)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_TPS_REAL_TIME_INFO));
        ...
        private void ProcessCommAlarm_PARK(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_ITS_PARK_VEHICLE struParkInfo = new CHCNetSDK_Linux.NET_ITS_PARK_VEHICLE();
            uint dwSize = (uint)Marshal.SizeOf(struParkInfo);
            struParkInfo = (CHCNetSDK_Linux.NET_ITS_PARK_VEHICLE)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_ITS_PARK_VEHICLE));
        ...
        private void ProcessCommAlarm_VQD(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_DIAGNOSIS_UPLOAD struVQDInfo = new CHCNetSDK_Linux.NET_DVR_DIAGNOSIS_UPLOAD();
            uint dwSize = (uint)Marshal.SizeOf(struVQDInfo);
            struVQDInfo = (CHCNetSDK_Linux.NET_DVR_DIAGNOSIS_UPLOAD)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_DIAGNOSIS_UPLOAD));
        ...
        private void ProcessCommAlarm_FaceSnap(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_VCA_FACESNAP_RESULT struFaceSnapInfo = new CHCNetSDK_Linux.NET_VCA_FACESNAP_RESULT();
            uint dwSize = (uint)Marshal.SizeOf(struFaceSnapInfo);
            struFaceSnapInfo = (CHCNetSDK_Linux.NET_VCA_FACESNAP_RESULT)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_VCA_FACESNAP_RESULT));
        ...
        private void ProcessCommAlarm_FaceMatch(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_VCA_FACESNAP_MATCH_ALARM struFaceMatchAlarm = new CHCNetSDK_Linux.NET_VCA_FACESNAP_MATCH_ALARM();
            uint dwSize = (uint)Marshal.SizeOf(struFaceMatchAlarm);
            struFaceMatchAlarm = (CHCNetSDK_Linux.NET_VCA_FACESNAP_MATCH_ALARM)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_VCA_FACESNAP_MATCH_ALARM));
        ...
        private void ProcessCommAlarm_FaceDetect(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_FACE_DETECTION struFaceDetectInfo = new CHCNetSDK_Linux.NET_DVR_FACE_DETECTION();
            uint dwSize = (uint)Marshal.SizeOf(struFaceDetectInfo);
            struFaceDetectInfo = (CHCNetSDK_Linux.NET_DVR_FACE_DETECTION)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_FACE_DETECTION));
        ...
        private void ProcessCommAlarm_CIDAlarm(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_CID_ALARM struCIDAlarm = new CHCNetSDK_Linux.NET_DVR_CID_ALARM();
            uint dwSize = (uint)Marshal.SizeOf(struCIDAlarm);
            struCIDAlarm = (CHCNetSDK_Linux.NET_DVR_CID_ALARM)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_CID_ALARM));
        ...
        private void ProcessCommAlarm_InterComEvent(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_VIDEO_INTERCOM_EVENT struInterComEvent = new CHCNetSDK_Linux.NET_DVR_VIDEO_INTERCOM_EVENT();
            uint dwSize = (uint)Marshal.SizeOf(struInterComEvent);
            struInterComEvent = (CHCNetSDK_Linux.NET_DVR_VIDEO_INTERCOM_EVENT)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_VIDEO_INTERCOM_EVENT));
        ...
        private void ProcessCommAlarm_AcsAlarm(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_ACS_ALARM_INFO struAcsAlarm = new CHCNetSDK_Linux.NET_DVR_ACS_ALARM_INFO();
            uint dwSize = (uint)Marshal.SizeOf(struAcsAlarm);
            struAcsAlarm = (CHCNetSDK_Linux.NET_DVR_ACS_ALARM_INFO)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_ACS_ALARM_INFO));
        ...
        private void ProcessCommAlarm_IDInfoAlarm(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_ID_CARD_INFO_ALARM struIDInfoAlarm = new CHCNetSDK_Linux.NET_DVR_ID_CARD_INFO_ALARM();
            uint dwSize = (uint)Marshal.SizeOf(struIDInfoAlarm);
            struIDInfoAlarm = (CHCNetSDK_Linux.NET_DVR_ID_CARD_INFO_ALARM)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_ID_CARD_INFO_ALARM));
        ...
        private void ProcessCommAlarm_AIOPVideo(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_AIOP_VIDEO_HEAD struAIOPVideo = new CHCNetSDK_Linux.NET_AIOP_VIDEO_HEAD();
            uint dwSize = (uint)Marshal.SizeOf(struAIOPVideo);
            struAIOPVideo = (CHCNetSDK_Linux.NET_AIOP_VIDEO_HEAD)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_AIOP_VIDEO_HEAD));
        ...
        private void ProcessCommAlarm_AIOPPicture(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_AIOP_PICTURE_HEAD struAIOPPic = new CHCNetSDK_Linux.NET_AIOP_PICTURE_HEAD();
            uint dwSize = (uint)Marshal.SizeOf(struAIOPPic);
            struAIOPPic = (CHCNetSDK_Linux.NET_AIOP_PICTURE_HEAD)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_AIOP_PICTURE_HEAD));
        ...
        private void ProcessCommAlarm_ISAPIAlarm(ref CHCNetSDK_Linux.NET_DVR_ALARMER pAlarmer, IntPtr pAlarmInfo, uint dwBufLen, IntPtr pUser)
        {
            CHCNetSDK_Linux.NET_DVR_ALARM_ISAPI_INFO struISAPIAlarm = new CHCNetSDK_Linux.NET_DVR_ALARM_ISAPI_INFO();
            uint dwSize = (uint)Marshal.SizeOf(struISAPIAlarm);
            struISAPIAlarm = (CHCNetSDK_Linux.NET_DVR_ALARM_ISAPI_INFO)Marshal.PtrToStructure(pAlarmInfo, typeof(CHCNetSDK_Linux.NET_DVR_ALARM_ISAPI_INFO));
